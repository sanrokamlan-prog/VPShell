...
- 桌面端已接通用户显式选择的长期 `russh` 终端：认证前固定 host-key，Rust-only 解析凭据，PTY 输入/输出、resize、取消、Shell Integration 与安全广播复用现有工作流；输出经 xterm 回执、有界队列和 SSH window 背压，迟到输出/退出受代际保护。系统 OpenSSH 继续是默认兼容路径。原生 SFTP 文件会话、逐跳凭据、跳板与端口转发仍待实现；
