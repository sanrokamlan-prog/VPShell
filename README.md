...
- 桌面端已接通用户显式选择的长期 `russh` 终端：认证前固定 host-key，Rust-only 解析凭据，PTY 输入/输出、resize、取消、Shell Integration 与安全广播复用现有工作流；同一已认证连接按需保持原生 SFTP 子系统供文件坞浏览，具备有界队列、超时、取消和代际保护。probe 与终端现统一使用最多四跳的有序 route 契约，每跳独立绑定 UUID、端点、SHA256 pin、超时及密码引用或私钥；当前执行仍 fail closed 地只允许一跳，跳板 tunnel 与端口转发尚未实现。大传输与远端变更继续使用独立连接，系统 OpenSSH 仍是默认兼容路径；
