...
VPShell 是 **Apache-2.0 开源、local-first 的 VPS/SSH 运维工作台**。连接资料、密钥引用、终端、SFTP 和用户自建的同步基础设施始终由用户控制；项目不出售 SSH 凭据、不用用户连接数据投放广告，也不会把核心客户端能力锁进付费版。

项目当前没有付费版、商业服务或已开通的官方赞助入口。未来可以通过 GitHub Sponsors 等**自愿赞助**承担维护成本，但赞助不会影响功能优先级、安全披露或开源版本的可用性。
