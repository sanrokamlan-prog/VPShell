...
> [!IMPORTANT]
> `v0.1.0-alpha.9` 是 Windows-first 技术预览版。本版包含跨重启传输恢复、远程文件操作、Linux 监控、Shell Integration、配置迁移和同步协议核心预览；Android 仅为独立预览工程。原生跳板仍需外部兼容验收，端到端同步、中继、端口转发和 Android 真机验收尚未完成。当前版本不应作为生产密码或私钥管理器。
...
- 桌面端已接通用户显式选择的长期 `russh` 终端：认证前固定 host-key，Rust-only 解析凭据，PTY 输入/输出、resize、取消、Shell Integration 与安全广播复用现有工作流；同一已认证连接按需保持原生 SFTP 子系统供文件坞浏览，具备有界队列、超时、取消和代际保护。probe 与终端使用最多四跳的有序 route；首跳 TCP 连接，后续逐跳通过上一会话的 `direct-tcpip` tunnel 建立独立 SSH 握手、SHA256 pin 与认证，整条连接链随终端统一关闭。添加主机界面当前可选择一台既有跳板，底层 route 支持最多三台跳板。大传输、外部编辑和远端变更继续使用独立兼容连接且尚未走跳板，端口转发仍待实现，系统 OpenSSH 仍是默认兼容路径；
