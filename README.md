...
- 设备 registry 最多 32 台，只记录公开签名键和非敏感标签；撤销单调、最后活动设备不可撤销、已撤销设备不能发布 registry。撤销不能抹除已复制的 VMK，疑似泄露时仍必须轮换主密钥并全量重加密；本机 outbox 使用系统凭据中的 Ed25519 私钥签署 operation 严格封套，远端签名 registry revision 链、SQLite 防回滚水位及协调器上传/下载授权验签已接线。桌面可生成 15 分钟、vault-bound 且由新设备私钥自签的登记请求，由现有活动设备按 expected revision 批准、改名或撤销并发布下一不可变 registry；当前发布设备不能自撤销。Android 仍只读显示 revision/本机授权/轮换提示且无管理 capability；Rust 已提供单对象及最多 10,000 项/256 MiB 的两阶段 VMK/CVK 批量重加密、provider 全量暂存、marker-last 密码/recovery 双 keyslot 激活、恢复打开和持久防回滚发现原语。协调器的 Rust-only RAII 守卫会在完整暂存/激活窗口冻结普通 provider 写入、配置和会话替换；未被任何认证 activation marker 引用的轮换 namespace 可由 Rust-only 条件删除清理，已激活轮换的旧 source objects 也可在新 namespace/旧 VMK 预检后条件回收，无法验证 marker、源对象或 provider 不支持条件删除时 fail closed；协调器自动切换和真实多设备矩阵仍未完成；
