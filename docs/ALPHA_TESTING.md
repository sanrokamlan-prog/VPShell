...
本指南适用于 `v0.1.0-alpha.1`。这是技术预览版，目标是尽快发现安装、兼容性、连接、传输和数据持久化问题，不是让测试者在生产环境替代现有 SSH 工具。

## 1. 下载与测试边界

- 只从 [VPShell GitHub Release](https://github.com/sanrokamlan-prog/VPShell/releases/tag/v0.1.0-alpha.1) 下载安装包。
- Windows 10/11 x64 优先使用 `VPShell_0.1.0-alpha.1_x64-setup.exe`；MSI 用于需要 MSI 部署验证的场景。
...
首次测试建议准备两台非生产 VPS：一台作为直连目标，另一台用于 ProxyJump 或 UDP 测速。远端准备一个仅用于测试的目录，例如 `~/vpshell-alpha-test`。
...
- 使用密钥或 SSH Agent 验证 ProxyJump，确认路线条显示“本机 -> 跳板机 -> 目标机”。
- 核对左侧主机 IP、用户、环境颜色和复制 IP 按钮。
- Linux 目标在非交互认证可用时，应显示 CPU、内存、磁盘、负载、流量和进程摘要；断开后停止采样。
- 在终端中手动再次执行 `ssh` 进入下一台主机时，当前 Alpha 不会自动识别新的嵌套主机，这是已知限制。
- ProxyJump 不会自动猜测或注入跳板机密码；优先使用密钥或 Agent。
...
- 当前不支持暂停、取消、断点续传和持久任务队列；SFTP/打包传输不支持 ProxyJump。
...
- 导入密码当前只供直连 SFTP 通过系统钥匙串使用，不会自动填入系统 OpenSSH 终端；终端认证仍需手动输入、私钥或 Agent。
...
> VPShell `v0.1.0-alpha.1` 正在进行小范围实机测试。请优先使用 Windows 10/11 x64 和非生产 VPS，重点测试安装启动、SSH 直连/跳板、多标签、最近连接、SFTP 文件与目录传输、拖放、打包传输、Notepad++ 外部编辑、FinalShell 导入、SSH 密钥、多机广播和网络诊断。请不要把它当作生产密码管理器，不要首次就在生产机执行广播或脚本。发现问题请写明系统版本、安装包、OpenSSH 版本、远端系统、认证/连接方式、复现步骤、预期和实际结果；截图和日志务必删除密码、私钥、Token、生产 IP、主机名及敏感命令输出。下载与反馈入口：https://github.com/sanrokamlan-prog/VPShell/releases/tag/v0.1.0-alpha.1
