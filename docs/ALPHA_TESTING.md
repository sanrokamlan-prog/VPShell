...
- 当前资料保存在本机 SQLite/受管资产缓存；桌面 Local Folder 与标准 HTTPS WebDAV 已把不可变 bootstrap、二级密码解锁、AppState 双向 operation 和 Rust 手动/自动周期接入设置页。初始化空存储后应确认 bootstrap 不含录入密码；“解锁已有”对缺失 bootstrap 失败，“初始化新 vault”对已有 bootstrap 失败，错误密码、未知版本、Local Folder 符号链接/特殊文件及 WebDAV 非 HTTPS/重定向/越界响应均 fail closed。WebDAV basic-auth 密码应只出现在系统凭据管理器，AppState 仅有 `sync-webdav-<UUID>` 引用；换密码失败不得丢失旧引用，配置失败的新引用应清理。为防止 AppState 异步保存失败破坏恢复能力，被成功替换的旧引用当前保留在本机 keyring，仍需后续显式清理入口。Android 只显示 value-free 恢复/冲突/队列状态且 Sync capability 保持禁用。真实 WebDAV 服务、自签 CA、代理、断网和平台目录兼容仍需外部矩阵；S3/SFTP 真实同步 transport 尚未实现。
