...
- 使用两个已配置同一 vault 的桌面测试设备往返 PNG、JPEG 和 WebP 背景，确认接收端只在全部加密分块和 manifest 认证后切换画面；分别移除一个分块和篡改密文应保持旧画面并给出稳定失败。JPEG/WebP 应保持原始编码但经过 Rust 结构校验，远端旧 blob 在活动设备确认式 GC 实现前应保守保留。
