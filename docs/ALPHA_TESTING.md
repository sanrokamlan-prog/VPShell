...
- 本地转发 IPC 不接受 `bindHost`、密码或 route 外凭据字段；监听必须固定为 `127.0.0.1`，0 端口由 OS 安全分配。Linux 双跳夹具的最终 sshd 只能 `PermitOpen` 自身测试端口，测试必须从本地 listener 读取真实 SSH banner、停止转发并等待 Rust 代际清理；非回环监听、远端与动态转发仍应拒绝或不可用。
