...
同步设置补充：`onboardingCompleted`、默认监控采样频率与背景可见度分别作为第三、第四、第五个固定 setting 实体，仅同步布尔完成状态、5–300 秒整数及 5%–65% 整数；公开命令历史使用独立稳定实体与 tombstone，含明显秘密的记录保持本机。引导内容、运行中监控采样、背景图片来源/资产和设备本地路径保持本机，远端回写不生成回声 change。
...
| 历史 | 命令、SFTP 路径和连接尝试历史由 Rust SQLite schema v9 快照/事件元数据管理；公开命令历史已接双向加密同步与删除，首次启动一次性迁移旧 WebView 状态 | SSH 进程启动不代表认证成功；路径/参数/连接历史同步尚未接线，终端内 `cd` 只由显式 Shell Integration 上报 |
...
| 同步 | Local Folder/WebDAV 显式初始化/解锁、E2EE 手动单周期与 Rust-owned 解锁期自动调度、WebDAV 本机 PEM CA、主机公开字段、安全自建脚本、五个固定设置实体、公开命令历史双向合并及持久冲突解决；Android 只读状态 | 尚无路径/参数/连接历史、背景图片 blob、其他尚未建模设置、自定义字体资产、扩展 provider 产品入口或真实多设备矩阵；自动调度不是应用关闭后的系统服务 |
...
本地业务状态 IPC 只接受 `InitializeAppStoreRequest`/`SaveAppStateRequest` 具名结构体。SQLite 使用 `user_version=6`、`app_state` 单例快照、不含值的 `app_events` 变更域、与状态保存同事务的有界 `app_sync_changes`，以及分别记录主机/脚本/setting vault、merge revision 和投影哈希的水位表。每次保存以 expected revision 防止覆盖，事务提交后再保留最多 10,000 条/90 天事件。主机 changefeed 只编码 name/address/port/username/group/environment/tags/jumpRoute；安全自建脚本只编码 name/body/source/risk/parameters；终端外观固定实体只编码 fontFamily/fontSize/lineHeight，应用行为偏好固定实体只编码 autoUploadEditedFiles/packageTransfersEnabled。主机与脚本以独立持久随机 UUID 映射本地 ID；credentialRef、私钥/路径、host-key pin、本机运行字段、内置脚本、脚本 description/category、未通过秘密扫描的脚本内容、`externalEditorPath` 及自定义字体资产/名称不会进入 operation，也不会被远端投影替换。超过 64 MiB 或 quick_check 失败时，Rust 按时间戳隔离原库并最多保留两个备份。状态校验拒绝未知顶层字段、过深/过大 JSON、控制字符、私钥正文和密码/Token/secret 等值字段。
...
`sync_provider` 已定义 Rust-only、有界、可取消的不可变 `list/get/put` 接口。Local Folder 逐级隔离符号链接，以同目录暂存、`fsync`、原子无覆盖 hard-link 和回读保持提交语义；WebDAV 强制 HTTPS/无重定向/有界显式 CA 和总超时，使用结构化 XML、条件 PUT 与提交后回读。对象最多 24 MiB，key/深度/分页/扫描/XML 均有硬限制，错误码区分取消、输入、路径、缺失、冲突、资源、服务和协议。`sync_coordinator` 已按 vault 作用域连接 provider、outbox、远端 receipt/head 与 merge。桌面 Local Folder 和 HTTPS WebDAV 产品入口都要求用户明确选择初始化或解锁：初始化以不可变 `vpshell/v1/bootstrap.json` 写入版本、vault ID 和 Argon2id 认证 keyslot，二级密码只在 Rust 清零内存中用于包裹/解包 VMK；解锁已有存储不会隐式创建新 vault。WebDAV basic-auth 密码以 `sync-webdav-<UUID>` 随机本机引用保存到系统凭据管理器，AppState 只持久化引用，协调器在 blocking worker 内解析并直接交给清零 provider credentials；引用、密码和 username 不进入状态事件或同步对象。显式 PEM CA 由 Rust 从用户选择的普通文件限量读取、解析并原子复制到应用私有目录，AppState 只保存 `sync-webdav-ca-<UUID>` 引用；源路径和 PEM 不进入持久状态、changefeed、事件或同步对象。桌面 capability 只开放 value-free 状态、受限配置、手动单周期、取消和锁定，以及 CA 资产安装/失败清理；自动调度复用同一协调器而不新增 WebView 启动命令，Android capability 继续排除桌面 Sync、provider 凭据与 CA 命令。

`sync_outbox` 在独立的 schema-v2 `vpshell-sync.sqlite3` 中原子写入加密 operation、outbox、merge state、本机 device seq 与单调 HLC。两分钟 claim 租约、最多六次 2 秒起/5 分钟封顶退避、显式暂停/恢复、不可逆发布态和过期租约恢复都由 SQLite 状态机拥有；前端不能提交或改写状态。远端对象在 Rust 内完成信封解析/AEAD 后，业务合并、receipt 和每设备连续序号 head 同事务提交；key/hash/对象身份唯一性阻止无序号对象换 key 重放。journal 的 10,000 未发布项/256 MiB、50,000 总对象/384 MiB、512 MiB 文件、30/90 天保留和两份损坏隔离备份均为硬边界。损坏恢复默认 `reconcile-required`，不会静默恢复上传。Local Folder worker 已接通 AppState 主机公开字段、安全自建脚本、五个固定设置实体与公开命令历史的本地入队和远端投影；投影要求本地 changefeed 已清空，并按实体域独立的 vault/revision/hash 在业务库事务提交，崩溃后从 journal merge state 重试。Rust 调度器在桌面解锁后执行 2 秒启动/业务变化防抖、5 分钟周期与 30 秒待处理或可重试复查；永久错误、取消和恢复阻止会暂停自动周期，锁定通过调度代际先停止旧 worker。路径/参数/连接历史、背景图片 blob、其他尚未建模设置、自定义字体资产、哈希链和设备签名仍未实现。

`sync_merge` 已实现 version 1 operation/state、HLC/device/operation 确定性字段 register、旧 history event 并集、因果 tombstone 和冲突中心。host/script/setting/background/history 只有逐字段类型/范围白名单；产品命令历史使用恰好五字段的 history 实体和 tombstone，不依赖缺少删除语义的旧 append event。host trust pin、credential ref、本机背景路径、密码/Token/私钥和敏感参数 history 不属于格式。本地主机、脚本、已接线设置与命令历史 patch 从当前 merge state 自动携带 observed stamps，并把 HLC 推进到已观察远端时钟之后；删除保存 observed-field stamps，使并发编辑/删除在任意到达顺序生成同一冲突。风险降低、连接身份和脚本正文变化也会持续显示。冲突解决自身使用 LWW stamp，支持保持删除或明确恢复。`sync_merge_state` 的 revision、apply 和写回由协调器嵌入本地 outbox 或 remote receipt 的同一 journal transaction；完整主机、脚本、五个固定 setting 与公开命令历史投影再以各自独立可重试事务写入 AppState，Android 仍只显示 revision/冲突数。桌面冲突中心只提交 revision、conflict ID 和候选索引，解决值由 Rust 从持久候选构造并加密入队。
