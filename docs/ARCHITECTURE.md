...
| 同步 | Local Folder/WebDAV 显式初始化/解锁、E2EE 手动单周期与 Rust-owned 解锁期自动调度、WebDAV 本机 PEM CA、主机公开字段、安全自建脚本、五个固定设置实体、PNG 规范化和 JPEG/WebP 结构校验背景分块、公开命令/路径/非敏感参数/已认证连接历史双向合并、活动设备确认式 blob GC 及持久冲突解决；Android 只读状态 | 其他尚未建模设置、自定义字体资产、扩展 provider 产品入口或真实多设备矩阵；GC 对不支持条件删除的 provider 保守保留，自动调度不是应用关闭后的系统服务 |
