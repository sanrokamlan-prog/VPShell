...
`sync_recovery` 已实现独立的 256-bit 可打印恢复密钥与 recovery keyslot，以及最多 32 台设备的 schema-v1 加密 registry。设备公钥身份不可替换，expected revision、撤销优先合并、最后活动设备保护和已撤销发布者拒绝防止本地静默复活。加密导出只封装 keyslot、认证密文和 manifest，限制为 10,000 对象/256 MiB 密文/384 MiB 文件；Rust 使用私有同目录暂存、文件与目录同步、hard-link 无覆盖提交，读取拒绝符号链接。恢复演练解包 VMK 后逐对象认证，严格解析 event 与 device registry。`sync_crypto` 的有界批量 VMK 原语会先认证同一 vault 的全部非重复对象并把明文放入清零容器，只有整个批次成功后才按原顺序生成新 nonce/密文；`sync_rotation` 进一步把完整 provider 快照写入隔离 namespace，以新 VMK 认证 manifest，并在重新认证新旧两侧后先发布新密码与 recovery keyslot、最后发布由当前 VMK 认证且绑定 revision/前序哈希/manifest/两个 keyslot 的 schema-v2 不可变 activation marker。密码或恢复读取均先验证两个 keyslot 的路径、vault、哈希与存在性，再以对应密钥解包并重验全部目标对象；旧 schema-v1 password-only marker 保持只读兼容。`discover_vault_rotation_activation` 逐页验证连续 marker 链并把最高已认证 revision/hash 持久写入独立 journal 水位，远端缺失、跳号、回退或分叉均 fail closed；桌面解锁以 bootstrap 对象哈希为链起点，在完整认证后自动切换 VMK，状态/UI 仅显示 revision 水位；marker 写失败留下的 keyslot 不可见。协调器以同一 runtime mutex 原子取得 Rust-only `VaultRotationGuard`，并在该守卫覆盖的完整暂存/激活窗口拒绝同步周期、配置/registry 写入、provider 会话替换和锁定；守卫释放后才解冻，因此最后一次现役快照认证与 activation marker 之间不存在普通协调器写入竞态。未被任何认证 activation marker 引用的 rotation namespace 可由 Rust-only 条件删除清理；已认证激活轮换的旧 source objects 可由 `cleanup_activated_rotation_sources` 在完整新 namespace 与旧 VMK 预检后按精确 bytes 条件删除，缺失对象可重试，marker、密文或 provider 条件删除任一失败均 fail closed。该边界不持久化恢复密钥、密码、私钥、provider 凭据或明文；远端 registry 回滚防护、协调器授权验签和桌面设备管理 UI 已接线，自动切换仅适用于 bootstrap 密码仍可解锁的同密码链；更改密码的恢复 UI 与真实多设备演练仍待后续。

`sync_credential_vault` 是与业务 VMK 分离的 Rust-only 可选层。策略默认关闭，expected revision 与 business device registry 共同控制最多 32 个活动/撤销授权，撤销单调且要求 CVK 轮换。独立随机 CVK 不可序列化/调试并清零，以 `credentials` 密码 keyslot 包裹，并由独立 `credential-recovery` HKDF/XChaCha keyslot 原语保护；模块提供单对象及最多 10,000 项/256 MiB 的批量 CVK 重加密，批次先完成授权、同 vault/非重复身份校验和全部旧 CVK 认证，之后才按原顺序生成新 nonce 并保留 item identity。SSH 密码、私钥口令、OpenSSH 私钥和 access token 分别进入独立 HKDF/AAD 域的严格认证信封。本机 credential reference 只用于一次性 Rust 读取且不写入信封/object key/诊断。内部系统钥匙串恢复只接受已授权并通过 AEAD 认证的 SSH 密码/私钥口令，为其生成不覆盖的随机 `ssh-`/`key-` 引用并回读验证，失败清理新条目；没有明确安装目标的私钥正文/access token fail closed。静态回归禁止该模块出现 Tauri command、事件或日志宏；provider/outbox、批量 CVK 恢复写回、轮换发布/提交、协调器与 UI 仍未实现。
