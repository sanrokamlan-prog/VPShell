...
Phase D 已接通桌面端第一个真实原生路径：锁定版本的 `russh + Tokio` 建立 SSH，`russh-sftp` 启动 SFTP 子系统并读取远端工作目录。当前入口是用户显式触发的就绪检查，不接管长期终端或文件面板；系统 OpenSSH 仍是默认兼容引擎。它用于先验证依赖、认证、主机密钥和 SFTP 互操作，再逐项扩大到长期会话、逐跳连接和端口转发。

就绪检查只接受具名、严格反序列化的请求。前端先通过既有信任库取得已验证的 SHA256 指纹；Rust 在任何认证之前重新比较服务器公钥，不匹配即硬失败。密码和私钥口令只由 Rust 从本机引用解析，私钥文件限制为无符号链接的绝对普通文件且不超过 1 MiB；host-key 和用户 RSA 认证都禁止 `ssh-rsa`/SHA-1，用户 RSA 私钥必须由服务器明确协商 SHA-2。并发最多 8 项，单项超时 5 至 60 秒，取消令牌和代际清理阻止迟到操作污染新状态；返回值只有 schema、引擎和 SSH/SFTP 就绪布尔值。
...
`russh` 仍处于 `0.x` 版本阶段，因此桌面依赖精确锁定，Linux CI 使用临时 Ed25519 用户和本机 OpenSSH/SFTP 做真实协议检查。该检查不证明长期 PTY 流控、多版本/旧服务器兼容、双跳板、逐跳换钥、转发或大流量；这些矩阵完成前不能将原生引擎设为默认，也不能移除系统 OpenSSH 回退。
