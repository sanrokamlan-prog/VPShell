...
Phase D 已接通桌面端真实原生路径：锁定版本的 `russh + Tokio` 建立 SSH，`russh-sftp` 启动 SFTP 子系统并读取远端工作目录；用户还可对单个未连接标签显式选择 `russh`，建立长期 PTY/Shell 终端。系统 OpenSSH 仍是默认兼容引擎，文件面板仍使用独立兼容 SFTP 连接；下一步再把原生连接扩展到共享 SFTP、逐跳连接和端口转发。

就绪检查和终端启动只接受具名、严格反序列化的请求。前端先通过既有信任库取得已验证的 SHA256 指纹；Rust 在任何认证之前重新比较服务器公钥，不匹配即硬失败。密码和私钥口令只由 Rust 从本机引用解析，私钥文件限制为无符号链接的绝对普通文件且不超过 1 MiB；host-key 和用户 RSA 认证都禁止 `ssh-rsa`/SHA-1，用户 RSA 私钥必须由服务器明确协商 SHA-2。probe 并发最多 8 项，长期终端最多 16 个；每个终端使用 64 项命令队列、64 项输出队列和 64 KiB 单次输入上限。原生输出带单调 `deliveryId`，xterm 完成解析后才经专用 IPC 确认；Rust 在确认前不抽取下一批事件，并在等待期间重投同一编号，前端按编号去重。各标签保留自己的终端消费者，切到后台只隐藏渲染面，不卸载输出链路。因此有界队列和 SSH window 会把慢消费压力传回远端而不丢字节，30 秒未确认则关闭会话。连接、PTY、Shell 均在返回“已连接”前确认，5 至 60 秒超时、取消令牌和双层代际清理阻止迟到任务或退出事件污染复用的标签。
...
`russh` 仍处于 `0.x` 版本阶段，因此桌面依赖精确锁定，Linux CI 使用临时 Ed25519 用户和本机 OpenSSH/SFTP 做真实协议检查，并实际打开 PTY、调整尺寸、收发终端字节和取消会话。该检查不证明多版本/旧服务器兼容、长时间大流量、双跳板、逐跳换钥或转发；这些矩阵完成前不能将原生引擎设为默认，也不能移除系统 OpenSSH 回退。
