...
| 历史 | 命令、SFTP 路径和连接尝试历史由 Rust SQLite schema v2 快照/事件元数据管理；首次启动一次性迁移旧 WebView 状态 | SSH 进程启动不代表认证成功；当前只有主机公开字段进入同步 operation，历史尚未接线，终端内 `cd` 只由显式 Shell Integration 上报 |
...
本地业务状态 IPC 只接受 `InitializeAppStoreRequest`/`SaveAppStateRequest` 具名结构体。SQLite 使用 `user_version=2`、`app_state` 单例快照、不含值的 `app_events` 变更域，以及与状态保存同事务的有界 `app_sync_changes`。每次保存以 expected revision 防止覆盖，事务提交后再保留最多 10,000 条/90 天事件。主机 changefeed 只编码 name/address/port/username/group/environment/tags/jumpRoute，并以持久随机 UUID 映射本地 ID；credentialRef、私钥/路径、host-key pin 和本机运行字段不会进入 operation。超过 64 MiB 或 quick_check 失败时，Rust 按时间戳隔离原库并最多保留两个备份。状态校验拒绝未知顶层字段、过深/过大 JSON、控制字符、私钥正文和密码/Token/secret 等值字段。
