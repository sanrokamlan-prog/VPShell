...
| 历史 | 命令、SFTP 路径和连接尝试历史由 Rust SQLite schema v5 快照/事件元数据管理；首次启动一次性迁移旧 WebView 状态 | SSH 进程启动不代表认证成功；历史同步尚未接线，终端内 `cd` 只由显式 Shell Integration 上报 |
...
| 同步 | Local Folder 显式初始化/解锁、E2EE 手动单周期、主机公开字段、安全自建脚本和终端字体族/字号/行高双向合并；Android 只读状态 | 尚无自动调度、冲突解决 UI、历史/背景/其余设置、自定义字体资产、WebDAV/扩展 provider 产品入口或真实多设备矩阵 |
...
本地业务状态 IPC 只接受 `InitializeAppStoreRequest`/`SaveAppStateRequest` 具名结构体。SQLite 使用 `user_version=5`、`app_state` 单例快照、不含值的 `app_events` 变更域、与状态保存同事务的有界 `app_sync_changes`，以及分别记录主机/脚本/终端外观 vault、merge revision 和投影哈希的水位表。每次保存以 expected revision 防止覆盖，事务提交后再保留最多 10,000 条/90 天事件。主机 changefeed 只编码 name/address/port/username/group/environment/tags/jumpRoute；安全自建脚本只编码 name/body/source/risk/parameters；终端外观使用固定协议实体，只编码 fontFamily/fontSize/lineHeight。主机与脚本以独立持久随机 UUID 映射本地 ID；credentialRef、私钥/路径、host-key pin、本机运行字段、内置脚本、脚本 description/category、未通过秘密扫描的脚本内容及自定义字体资产/名称不会进入 operation，也不会被远端投影替换。超过 64 MiB 或 quick_check 失败时，Rust 按时间戳隔离原库并最多保留两个备份。状态校验拒绝未知顶层字段、过深/过大 JSON、控制字符、私钥正文和密码/Token/secret 等值字段。
