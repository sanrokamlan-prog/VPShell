...
同步设置补充：`onboardingCompleted`、默认监控采样频率与背景可见度分别作为第三、第四、第五个固定 setting 实体，仅同步布尔完成状态、5–300 秒整数及 5%–65% 整数；引导内容、运行中监控采样、背景图片来源/资产和设备本地路径保持本机，远端回写不生成回声 change。
...
| 历史 | 命令、SFTP 路径和连接尝试历史由 Rust SQLite schema v8 快照/事件元数据管理；首次启动一次性迁移旧 WebView 状态 | SSH 进程启动不代表认证成功；历史同步尚未接线，终端内 `cd` 只由显式 Shell Integration 上报 |
...
| 同步 | Local Folder/WebDAV 显式初始化/解锁、E2EE 手动单周期与 Rust-owned 解锁期自动调度、WebDAV 本机 PEM CA、主机公开字段、安全自建脚本、五个固定设置实体双向合并及持久冲突解决；Android 只读状态 | 尚无历史/背景图片 blob/其他尚未建模设置、自定义字体资产、扩展 provider 产品入口或真实多设备矩阵；自动调度不是应用关闭后的系统服务 |
