...
| 同步 | Local Folder/WebDAV 显式初始化/解锁、E2EE 手动单周期与 Rust-owned 解锁期自动调度、主机公开字段、安全自建脚本、四个固定设置实体双向合并及持久冲突解决；Android 只读状态 | 尚无历史/背景/其他尚未建模设置、自定义字体资产、扩展 provider 产品入口、WebDAV 自签 CA 入口或真实多设备矩阵；自动调度不是应用关闭后的系统服务 |
...
`sync_provider` 已定义 Rust-only、有界、可取消的不可变 `list/get/put` 接口。Local Folder 逐级隔离符号链接，以同目录暂存、`fsync`、原子无覆盖 hard-link 和回读保持提交语义；WebDAV 强制 HTTPS/无重定向/有界显式 CA 和总超时，使用结构化 XML、条件 PUT 与提交后回读。对象最多 24 MiB，key/深度/分页/扫描/XML 均有硬限制，错误码区分取消、输入、路径、缺失、冲突、资源、服务和协议。`sync_coordinator` 已按 vault 作用域连接 provider、outbox、远端 receipt/head 与 merge。桌面 Local Folder 和标准 HTTPS WebDAV 产品入口都要求用户明确选择初始化或解锁：初始化以不可变 `vpshell/v1/bootstrap.json` 写入版本、vault ID 和 Argon2id 认证 keyslot，二级密码只在 Rust 清零内存中用于包裹/解包 VMK；解锁已有存储不会隐式创建新 vault。WebDAV basic-auth 密码以 `sync-webdav-<UUID>` 随机本机引用保存到系统凭据管理器，AppState 只持久化引用，协调器在 blocking worker 内解析并直接交给清零 provider credentials；引用、密码和 username 不进入状态事件或同步对象。当前产品入口使用系统 CA 和固定 30 秒上限，显式自签 CA 仍未开放。桌面 capability 只开放 value-free 状态、受限配置、手动单周期、取消和锁定；自动调度复用同一协调器而不新增 WebView 启动命令，Android capability 继续排除桌面 Sync 与 provider 凭据命令。
