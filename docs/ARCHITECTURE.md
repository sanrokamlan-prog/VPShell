...
就绪检查和终端启动只接受具名、严格反序列化的 `route.hops[]` 请求。route 限制 1–4 跳且禁止重复 hop UUID 或端点；每跳独立携带 host/user/port、5–60 秒超时、已验证的 SHA256 pin，以及密码引用或私钥二选一的认证来源。Rust 先验证整个无秘密 route，再只为即将连接的 hop 解析本机凭据；错误只补充 1-based hop 序号，不回显地址、路径或引用。当前 connector 明确拒绝两跳以上 route，因此该契约不能被当成已实现跳板。
