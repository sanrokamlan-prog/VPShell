...
同步设置补充：`onboardingCompleted`、默认监控采样频率与背景可见度分别作为第三、第四、第五个固定 setting 实体，仅同步布尔完成状态、5–300 秒整数及 5%–65% 整数；公开命令/路径/参数历史使用独立稳定实体与 tombstone，参数还必须绑定当前命令模板声明的非敏感字段，含明显秘密或没有真实时间的旧记录保持本机。引导内容、运行中监控采样、背景图片来源/资产和设备本地路径保持本机，远端回写不生成回声 change。
...
| 历史 | 命令、参数、SFTP 路径和连接尝试历史由 Rust SQLite schema v9 快照/事件元数据管理；公开命令/路径及模板非敏感参数历史已接双向加密同步与删除，首次启动一次性迁移旧 WebView 状态 | SSH 进程启动不代表认证成功；连接历史同步尚未接线，终端内 `cd` 只由显式 Shell Integration 上报 |
...
| 同步 | Local Folder/WebDAV 显式初始化/解锁、E2EE 手动单周期与 Rust-owned 解锁期自动调度、WebDAV 本机 PEM CA、主机公开字段、安全自建脚本、五个固定设置实体、公开命令/路径/非敏感参数历史双向合并及持久冲突解决；Android 只读状态 | 尚无连接历史、背景图片 blob、其他尚未建模设置、自定义字体资产、扩展 provider 产品入口或真实多设备矩阵；自动调度不是应用关闭后的系统服务 |
...
本地业务状态 IPC 只接受 `InitializeAppStoreRequest`/`SaveAppStateRequest` 具名结构体。SQLite 使用 `user_version=9`、`app_state` 单例快照、不含值的 `app_events` 变更域、与状态保存同事务的有界 `app_sync_changes`，以及分别记录主机/脚本/setting/history vault、merge revision 和投影哈希的水位表。每次保存以 expected revision 防止覆盖，事务提交后再保留最多 10,000 条/90 天事件。主机 changefeed 只编码 name/address/port/username/group/environment/tags/jumpRoute；安全自建脚本只编码 name/body/source/risk/parameters；终端外观固定实体只编码 fontFamily/fontSize/lineHeight，应用行为偏好固定实体只编码 autoUploadEditedFiles/packageTransfersEnabled。主机与脚本以独立持久随机 UUID 映射本地 ID；history 使用按 kind 隔离的稳定身份与内容指纹。credentialRef、私钥/路径、host-key pin、本机运行字段、内置脚本、脚本 description/category、未通过秘密扫描的脚本内容、`externalEditorPath` 及自定义字体资产/名称不会进入 operation，也不会被远端投影替换。超过 64 MiB 或 quick_check 失败时，Rust 按时间戳隔离原库并最多保留两个备份。状态校验拒绝未知顶层字段、过深/过大 JSON、控制字符、私钥正文和密码/Token/secret 等值字段。
...
`sync_outbox` 在独立的 schema-v2 `vpshell-sync.sqlite3` 中原子写入加密 operation、outbox、merge state、本机 device seq 与单调 HLC。两分钟 claim 租约、最多六次 2 秒起/5 分钟封顶退避、显式暂停/恢复、不可逆发布态和过期租约恢复都由 SQLite 状态机拥有；前端不能提交或改写状态。远端对象在 Rust 内完成信封解析/AEAD 后，业务合并、receipt 和每设备连续序号 head 同事务提交；key/hash/对象身份唯一性阻止无序号对象换 key 重放。journal 的 10,000 未发布项/256 MiB、50,000 总对象/384 MiB、512 MiB 文件、30/90 天保留和两份损坏隔离备份均为硬边界。损坏恢复默认 `reconcile-required`，不会静默恢复上传。Local Folder worker 已接通 AppState 主机公开字段、安全自建脚本、五个固定设置实体与公开命令/路径/非敏感参数历史的本地入队和远端投影；投影要求本地 changefeed 已清空，并按实体域独立的 vault/revision/hash 在业务库事务提交，崩溃后从 journal merge state 重试。Rust 调度器在桌面解锁后执行 2 秒启动/业务变化防抖、5 分钟周期与 30 秒待处理或可重试复查；永久错误、取消和恢复阻止会暂停自动周期，锁定通过调度代际先停止旧 worker。连接历史、背景图片 blob、其他尚未建模设置、自定义字体资产、哈希链和设备签名仍未实现。

`sync_merge` 已实现 version 1 operation/state、HLC/device/operation 确定性字段 register、旧 history event 并集、因果 tombstone 和冲突中心。host/script/setting/background/history 只有逐字段类型/范围白名单；产品命令历史使用恰好五字段、路径历史使用恰好四字段、参数历史使用恰好五字段的 history 实体和 tombstone，不依赖缺少删除语义的旧 append event。参数实体绑定 `commandId` 与 `parameterName`，AppStore 还要求当前命令模板声明同名非敏感字段；host trust pin、credential ref、本机背景路径、密码/Token/私钥和敏感参数 history 不属于格式。本地主机、脚本、已接线设置与命令/路径/参数历史 patch 从当前 merge state 自动携带 observed stamps，并把 HLC 推进到已观察远端时钟之后；删除保存 observed-field stamps，使并发编辑/删除在任意到达顺序生成同一冲突。风险降低、连接身份和脚本正文变化也会持续显示。冲突解决自身使用 LWW stamp，支持保持删除或明确恢复。`sync_merge_state` 的 revision、apply 和写回由协调器嵌入本地 outbox 或 remote receipt 的同一 journal transaction；完整主机、脚本、五个固定 setting 与公开 history 投影再以各自独立可重试事务写入 AppState，Android 仍只显示 revision/冲突数。桌面冲突中心只提交 revision、conflict ID 和候选索引，解决值由 Rust 从持久候选构造并加密入队。
