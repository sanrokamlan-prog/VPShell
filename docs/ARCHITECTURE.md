...
| SSH | Rust 后端默认通过 `portable-pty` 启动系统 `ssh`；可选 `russh` route 提供逐跳 pin/认证、PTY/SFTP 与本地/远端回环转发 | 系统路径依赖 `ssh` 在 PATH 中；原生路径仍缺动态转发和广泛服务器兼容验证 |
...
Phase D 已接通桌面端真实原生路径：锁定版本的 `russh + Tokio` 建立 SSH；用户可对单个未连接标签显式选择 `russh`，建立长期 PTY/Shell 终端，并让文件坞的目录浏览按需打开、持续复用同一已认证连接上的 `russh-sftp` 子系统。应用管理的 route 可通过 `direct-tcpip` 逐跳建立 tunnel；本地端口转发固定监听本机 `127.0.0.1`，每条 route 连接在最终 SSH 会话上为本地 TCP 连接打开独立 `direct-tcpip` channel。远端转发只向最终 SSH 会话请求 `127.0.0.1` 监听，并把经过验证的 `forwarded-tcpip` channel 接到客户端本机 `127.0.0.1` 目标；未登记、地址/端口不匹配、取消或容量耗尽的 channel 在确认前拒绝。系统 OpenSSH 仍是默认兼容引擎。大传输、外部编辑和远端变更继续使用独立兼容 SFTP 连接，避免共享通道阻塞终端，也保留既有恢复/原子提交语义，但这些独立连接尚未支持跳板。动态转发仍待实现。
...
`russh` 仍处于 `0.x` 版本阶段，因此桌面依赖精确锁定。Linux CI 启动具有不同 host key、用户和客户端私钥的回环跳板与目标 OpenSSH：跳板只允许到目标测试端口的 `direct-tcpip`；目标的 `PermitOpen` 唯一指向自身测试端口，`PermitListen` 只允许 `127.0.0.1:*` 且关闭 `GatewayPorts`。测试经 tunnel 完成第二跳 pin、公钥认证、SFTP、PTY、字节收发和取消；本地转发从 Rust-owned 回环监听读取 OpenSSH banner，远端转发使用服务器分配的回环端口把同一 banner 送回客户端回环目标并显式取消。该检查不证明多版本/旧服务器兼容、长时间大流量、两台以上跳板、逐跳换钥、非回环监听或动态转发；这些矩阵完成前不能将原生引擎设为默认，也不能移除系统 OpenSSH 回退。
