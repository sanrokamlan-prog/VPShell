...
同步设置补充：`onboardingCompleted`、默认监控采样频率与背景可见度分别作为第三、第四、第五个固定 setting 实体，仅同步布尔完成状态、5–300 秒整数及 5%–65% 整数；公开命令/路径/参数历史使用独立稳定实体与 tombstone，参数还必须绑定当前命令模板声明的非敏感字段，含明显秘密或没有真实时间的旧记录保持本机。规范化 PNG 背景使用独立 background 实体引用 Rust-owned 加密分块；原始 URL、JPEG/WebP、引导内容、运行中监控采样和设备本地路径保持本机，远端回写不生成回声 change。
...
| 同步 | Local Folder/WebDAV 显式初始化/解锁、E2EE 手动单周期与 Rust-owned 解锁期自动调度、WebDAV 本机 PEM CA、主机公开字段、安全自建脚本、五个固定设置实体、规范化 PNG 背景分块、公开命令/路径/非敏感参数/已认证连接历史双向合并及持久冲突解决；Android 只读状态 | 尚无 JPEG/WebP 背景、确认式远端 blob 回收、其他尚未建模设置、自定义字体资产、扩展 provider 产品入口或真实多设备矩阵；自动调度不是应用关闭后的系统服务 |
...
`sync_outbox` 在独立的 schema-v2 `vpshell-sync.sqlite3` 中原子写入加密 operation、outbox、merge state、本机 device seq 与单调 HLC。两分钟 claim 租约、最多六次 2 秒起/5 分钟封顶退避、显式暂停/恢复、不可逆发布态和过期租约恢复都由 SQLite 状态机拥有；前端不能提交或改写状态。远端对象在 Rust 内完成信封解析与 AEAD 认证后，业务合并、receipt 和每设备连续序号 head 同事务提交；key/hash/对象身份唯一性阻止无序号对象换 key 重放。journal 的 10,000 未发布项/256 MiB、50,000 总对象/384 MiB、512 MiB 文件、30/90 天保留和两份损坏隔离备份均为硬边界。损坏恢复默认 `reconcile-required`，不会静默恢复上传。Local Folder worker 已接通 AppState 主机公开字段、安全自建脚本、五个固定设置实体、规范化 PNG 背景分块与公开命令/路径/非敏感参数/已认证连接历史的本地入队和远端投影；连接条目还必须匹配 Rust 认证事实。PNG 资产先于引用 operation 发布；journal 在同一 vault 尚有未发布 blob 时持久阻止 event 领取，因此分块退避也不能绕过该顺序。远端 manifest/全部块完成 AEAD、大小和 SHA-256 校验并由 PNG 解码器复核后，才原子替换本机资产并推进 schema-v11 background 投影。投影要求本地 changefeed 已清空，并按实体域独立的 vault/revision/hash 在业务库事务提交，崩溃后从 journal merge state 重试。Rust 调度器在桌面解锁后执行 2 秒启动/业务变化防抖、5 分钟周期与 30 秒待处理或可重试复查；永久错误、取消和恢复阻止会暂停自动周期，锁定通过调度代际先停止旧 worker。JPEG/WebP 背景、确认式远端 blob 回收、其他尚未建模设置、自定义字体资产、哈希链和设备签名仍未实现。
