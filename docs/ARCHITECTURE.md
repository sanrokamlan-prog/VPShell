...
| 同步 | Local Folder/WebDAV 显式初始化/解锁、E2EE 手动单周期与 Rust-owned 解锁期自动调度、WebDAV 本机 PEM CA、主机公开字段、安全自建脚本、四个固定设置实体双向合并及持久冲突解决；Android 只读状态 | 尚无历史/背景/其他尚未建模设置、自定义字体资产、扩展 provider 产品入口或真实多设备矩阵；自动调度不是应用关闭后的系统服务 |
