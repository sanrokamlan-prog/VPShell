...
- 当前模块不暴露 Tauri command/event，尚未接入设置 UI、provider/outbox、协调器、CVK 恢复写回或轮换；`credential-recovery` keyslot 仅是内部密码学原语，因此应用仍不会同步凭据。
...
5. **恢复与设备层**：内部可打印恢复密钥、业务 VMK recovery keyslot、独立 CVK recovery keyslot、单调设备撤销、加密导出和离线恢复演练已实现；仍需设备签名、轮换、协调器/UI 与真实多设备演练。
...
8. **凭据 vault**：默认关闭、独立 CVK/keyslot/对象域、`credential-recovery` keyslot、逐设备授权及 SSH 密码/私钥口令内部钥匙串恢复原语已完成；仍需 provider/outbox、恢复写回流程、CVK/VMK 轮换、恢复演练、协调器/UI 与真实设备验证。
