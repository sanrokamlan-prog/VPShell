...
当前 `sync_outbox.rs` 使用独立 `vpshell-sync.sqlite3` schema v5 落实 journal 内原子边界，并持久保存随机本机 device ID、单调 HLC、每个 vault 的已验证 registry revision/哈希/签名信封与 rotation activation revision/hash。调用者只能通过 Rust 内部闭包修改同一个 SQLite transaction；闭包不得执行网络、文件写入或其他不可回滚副作用。`sync_operations` 只保存已通过 v1 严格解析的加密信封，`sync_outbox` 保存状态/次数/租约/稳定错误码，业务明文、密码、私钥、credential ref、Token 和 provider 凭据不进入 journal。未发布最多 10,000 对象/256 MiB，整个 journal 最多 50,000 对象/384 MiB，数据库文件启动硬限制 512 MiB；已发布本地对象保留 30 天，远端 receipt 保留 90 天，但每设备持久高水位不会清理。协调器状态只暴露已认证的 activation revision，不暴露密钥或密码；轮换切换仍需后续完成 provider namespace 映射与恢复 UI，不能把该水位误报为已启用新 VMK。
...
vpshell/v1/<vault_id>/
  ...
  rotations/<rotation_id>/<staged_object_or_manifest_or_password_or_recovery_keyslot>
...
- rotation namespace 只保存新 VMK 下的暂存对象、manifest，以及分别由新密码/离线恢复密钥包裹的 keyslot；只有连续 revision 的 activation commit 才构成逻辑切换点；
...
- 从 OS CSPRNG 生成独立 256-bit CVK，类型不可 Serialize/Debug 且释放清零；CVK 以独立密码和 `credentials` keyslot/AAD 域包裹，并有独立 `credential-recovery` HKDF/XChaCha keyslot 原语，不复用业务 VMK 或 recovery keyslot；
...
- `sync_rotation::publish_vault_rotation` 是同样的 Rust-only 暂存发布边界：它分页认证同一 vault 的 segments、blobs、registry 和 blob-GC index，先一次性完成批量重加密，再把新对象写入不可覆盖的 `rotations/{rotation_id}/` namespace，最后发布新 VMK 加密 manifest。`activate_vault_rotation` 在任何激活副作用前重验 manifest、全部新密文和未漂移的现役旧快照，随后分别发布只包裹新 VMK 的随机密码 keyslot 与 recovery keyslot，并把由当前 VMK 认证、绑定 revision/前序 activation hash/manifest hash/两个 keyslot 路径和哈希的 schema-v2 `activations/<revision>.orac` 放在最后；该单一 marker 是逻辑切换点。密码和恢复读取都会先验证两个绑定 keyslot 完整存在且路径、vault 与哈希匹配，使用对应密钥解包后再以新 VMK 重验 manifest 和全部目标对象才返回新密钥；旧 schema-v1 password-only marker 保持只读兼容但不能提供恢复打开。`discover_vault_rotation_activation` 逐页枚举并验证连续 marker 链，把最高已认证 revision/hash 写入独立 journal 水位；远端缺失、跳号、回退或分叉均 fail closed。协调器仍只暴露已认证水位，不能在没有 source-to-rotation namespace 映射、keyslot/恢复输入和完整新对象 overlay 的情况下自动换用新 VMK。协调器只通过 `VaultRotationGuard::publish_and_activate` 进入完整发布/激活流程；该 RAII 守卫在同一 runtime mutex 临界区冻结普通同步、配置/registry 写入和会话替换，直到 marker 成功或错误返回后释放。未被任何认证 activation marker 引用的轮换 namespace 可由 Rust-only `cleanup_orphaned_rotation` 使用条件删除清理；已认证激活轮换的旧 source objects 可由 `cleanup_activated_rotation_sources` 在完整新 namespace 与旧 VMK 预检后按精确 bytes 条件删除，缺失对象可重试，marker、密文或 provider 条件删除任一失败均 fail closed。marker 无法认证或 provider 不支持条件删除时 fail closed。marker 写入失败最多留下不可见的孤儿 keyslot，不会激活轮换；恢复 UI 与真实多设备演练仍待后续。
...
5. **恢复与设备层**：内部可打印恢复密钥、业务 VMK/CVK recovery keyslot、单调设备撤销、单对象及有界两阶段 VMK/CVK 批量重加密、provider 全量暂存、marker-last 密码/recovery 双 keyslot 激活、恢复打开、连续 activation 发现、持久防回滚水位、协调器并发写冻结、未引用轮换 namespace 清理及已激活轮换旧 source object 条件回收、加密导出及离线恢复演练已实现；仍需协调器自动切换/UI 与真实多设备演练。
