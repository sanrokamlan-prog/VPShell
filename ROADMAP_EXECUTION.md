...
| 状态 | 验收项 |
| --- | --- |
| 进行中 | D1 已接通桌面 `russh`/`russh-sftp` 真实就绪检查、用户显式选择的长期 PTY/Shell 终端、同一已认证连接拥有的长期 SFTP 目录浏览会话及 Rust-owned 本地回环端口转发；认证前固定 host-key、Rust-only 凭据解析、xterm 输出回执、有界背压/超时/并发、取消/代际保护及回环 OpenSSH/SFTP/终端 CI 已覆盖。probe/终端/本地转发使用最多四跳、逐跳独立 pin/认证来源的有序 route；首跳 TCP、后续通过上一跳 `direct-tcpip` channel stream 建立独立 SSH 会话，整条链统一持有和逆序关闭。添加主机界面可选择一台已有跳板，模型支持最多三台跳板；大传输/变更仍为不支持跳板的独立连接，远端与动态端口转发待实现 |
...
| 2026-08-17 | D1 Rust-owned 本地回环端口转发（待 Actions） | 新增 desktop-only `start/list/stop_native_local_forward`：请求只接受已验证 route、本地端口和目标 host/port，Rust 固定监听 `127.0.0.1`，0 端口由 OS 分配；最多 8 条转发、每条 32 个并发连接。每个本地 TCP 连接只在最终已认证 SSH 会话打开目标受限 `direct-tcpip` channel，以 Tokio 双向复制字节；活动/累计/拒绝计数为无流量内容快照。停止或 listener 失败会中止 socket/channel、逆序断开 route 并按 generation 清理，错误不包含端点、凭据或底层文本。桌面对话框可启动、刷新和停止；Android manifest/capability 明确无转发命令。单测覆盖未知监听地址/秘密字段、目标/UUID/容量、计数、取消与代际；Linux 双 sshd 最终节点仅允许 local forwarding 且唯一 `PermitOpen` 为自身测试端口，真实测试经两跳本地 listener 读取 OpenSSH banner 后停止并等待清理。远端与动态转发仍未实现。本机 `git diff --check`、严格 JSON、76-command handler/capability/Android 分离、workflow fixture `bash -n`、本机 `sshd -T` 选项解析及回环/容量/双向复制静态门禁通过；无 Cargo/rustfmt/node_modules/target，根分区 45%。等待 Actions 编译、完整测试和真实本地转发 fixture。 |
...
等待 D1 本地回环端口转发提交的新 PR #1 Actions 全部进入 `COMPLETED/SUCCESS`；失败则继续定位并修复。全绿后分项实现远端端口转发，再实现动态 SOCKS 转发。C4 真机泄漏矩阵保持外部验收；Android Sync capability 继续 disabled。
