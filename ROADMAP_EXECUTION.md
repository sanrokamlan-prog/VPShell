...
| 2026-08-17 | D1 Rust-owned 远端回环端口转发（待 Actions） | 新增 desktop-only `start/list/stop_native_remote_forward`：请求只接受已验证 route、远端端口和本机目标端口，最终 SSH hop 仅请求 `127.0.0.1` 监听，客户端目标同样固定 `127.0.0.1`，不接受地址或秘密字段。最终 hop handler 只确认已登记且地址/端口匹配的 `forwarded-tcpip` channel；取消、未就绪、队列满或 32 连接满均在确认前拒绝，等待 accept 时取消仍优先。最多 8 条，支持服务器安全分配 0 端口；停止先发送 `cancel-tcpip-forward`，再中止活动 channel 并逆序关闭 route，状态只返回活动/累计/拒绝/目标失败计数。桌面端口转发对话框提供本地/远端模式；Android capability 明确排除新增三命令。单测覆盖未知字段、端口分配响应、容量、取消、计数与代际；Linux 双 sshd 以 `PermitListen=127.0.0.1:*`、`GatewayPorts=no` 经两跳远端 listener 把真实 OpenSSH banner 转回本机回环目标并等待取消清理。本机 `git diff --check`、严格 JSON、79-command handler/capability/Android 分离、fixture `bash -n` 与本机 `sshd -T` 配置解析通过；无 Cargo/rustfmt/node_modules/target，根分区 45%。动态 SOCKS 仍未实现，等待 Actions 编译、完整测试和真实 fixture。 |

## 下一个动作

等待 D1 远端回环端口转发提交的新 PR #1 Actions 全部进入 `COMPLETED/SUCCESS`；失败则继续定位并修复。全绿后实现动态 SOCKS 转发。C4 真机泄漏矩阵保持外部验收；Android Sync capability 继续 disabled。
