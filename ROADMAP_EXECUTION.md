...
| 进行中 | 用户自建 Relay 参考实现、认证协议、限流、审计和安全测试：desktop-only 可运行 serve/loopback connect、challenge-bound HMAC-SHA256 双向 proof、精确目标 allowlist、token/audit 私有文件、全局/单 IP/认证/字节/空闲/总时长硬上限、盐哈希 JSONL 审计和真实回环 TCP 测试已实现，等待 Actions；真实部署与 TLS/VPN 为外部验收 |
...
等待自建 Relay 格式修复提交的 PR #1 Actions 全部进入 `COMPLETED/SUCCESS`；失败则继续定位并修复。全绿后进入持续测速、可解释选路与可选 Mosh。C4 真机泄漏矩阵、Relay 真实公网/多区域/TLS-VPN 部署保持外部验收；Android Sync capability 继续 disabled。
