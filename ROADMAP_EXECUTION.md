...
| 状态 | 验收项 |
| --- | --- |
| 进行中 | D1 已接通桌面 `russh`/`russh-sftp` 真实就绪检查、用户显式选择的长期 PTY/Shell 终端及同一已认证连接拥有的长期 SFTP 目录浏览会话；认证前固定 host-key、Rust-only 凭据解析、xterm 输出回执、有界背压/超时/并发、取消/代际保护及回环 OpenSSH/SFTP/终端 CI 已覆盖。probe/终端已统一为最多四跳、逐跳独立 pin/认证来源的有序 route 契约，但执行仍只允许单跳；大传输/变更按架构保持独立连接，跳板 tunnel 和端口转发仍待实现 |
...
| 2026-08-17 | D1 逐跳凭据与 host-key route 契约（待 Actions） | `native_engine_probe` 与 `start_native_terminal` 从扁平连接字段切换为严格 deserialize-only `route.hops[]`，限制 1–4 跳并拒绝重复 hop UUID/host-port；每跳独立绑定 host/user/port、5–60 秒超时、SHA256 pin 和密码引用或私钥二选一。Rust 先完成无秘密 route 验证，实际连接该跳时才解析本机秘密；握手、pin、认证错误只附 1-based `hopIndex`，不返回端点、引用、路径或底层错误。前端真实单跳 probe/终端与 Linux 回环 fixture 均改走新 route；两跳以上执行稳定 fail closed 并提示继续使用系统 OpenSSH，未冒充跳板。单测覆盖独立 password/key hop、不同 pin、空/超限/重复 route、旧扁平与未知秘密字段拒绝、hopIndex 无值错误。本机 `git diff --check`、JSON、73-command handler/capability 分离、deserialize-only route、延迟秘密解析、fail-closed connector 和前端接线静态检查通过；无 Cargo/rustfmt/node_modules/target，根分区 45%，等待 Actions 编译、完整测试和真实 fixture。 |
...
| 2026-08-17 | D1 逐跳 route Windows 测试路径修复 | 格式修复提交 `0ebf720` 的 PR #1 run `32076665444` 已进入终态：frontend、Ubuntu、macOS Intel、macOS arm、Android aarch64 debug APK/Gradle gate 全部 `COMPLETED/SUCCESS`，Linux 真实 route/OpenSSH/SFTP/终端 fixture 通过；Windows 的 fmt/check 及 177/178 tests 通过，唯一失败为 route 独立私钥 hop 测试写死 Unix `/tmp/target-key`，Windows 正确拒绝其为非绝对路径。测试现改用平台原生临时目录生成绝对路径，不读取或创建私钥、不改变生产校验语义；本机 `git diff --check`、JSON、73-command capability 分离、route portability/fail-closed 静态检查通过，无 Cargo/rustfmt/node_modules/target，根分区 45%。等待修复提交后的完整 Actions 矩阵。 |

## 下一个动作

等待 D1 逐跳 route 契约提交的新 PR #1 Actions 全部进入 `COMPLETED/SUCCESS`；失败则继续定位并修复。全绿后实现真实跳板 tunnel，再分项实现本地/远端/动态端口转发。C4 真机泄漏矩阵保持外部验收；Android Sync capability 继续 disabled。
