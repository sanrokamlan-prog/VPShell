...
| 进行中 | Rust-owned 持续 route readiness 测量与可解释选路建议：真实比较同一目标直连/已配置跳板的逐跳认证、host-key pin 和最终 SFTP readiness，滚动成功率/中位数/P95/失败惩罚、可靠性门槛、切换滞后、取消/代际及脱敏快照已接线，等待 Actions；不自动改写 route，不称“加速” |
...
等待 route readiness 持续评估提交的 PR #1 Actions 全部进入 `COMPLETED/SUCCESS`；失败则继续定位并修复。全绿后进入可选 Mosh 独立交互模式。C4 真机泄漏矩阵、路线评估真实双路径长时间测试、Relay 真实公网/多区域/TLS-VPN 部署保持外部验收；Android Sync capability 继续 disabled。
