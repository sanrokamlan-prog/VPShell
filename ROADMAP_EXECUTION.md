...
| 状态 | 验收项 |
| --- | --- |
| 进行中 | D1 已接通桌面 `russh`/`russh-sftp` 真实就绪检查、用户显式选择的长期 PTY/Shell 终端、同一已认证连接拥有的长期 SFTP 目录浏览会话及 Rust-owned 本地/远端回环端口转发；认证前固定 host-key、Rust-only 凭据解析、xterm 输出回执、有界背压/超时/并发、取消/代际保护及回环 OpenSSH/SFTP/终端 CI 已覆盖。probe/终端/两类转发使用最多四跳、逐跳独立 pin/认证来源的有序 route；首跳 TCP、后续通过上一跳 `direct-tcpip` channel stream 建立独立 SSH 会话，整条链统一持有和逆序关闭。添加主机界面可选择一台已有跳板，模型支持最多三台跳板；大传输/变更仍为不支持跳板的独立连接，动态 SOCKS 转发待实现 |
...
| 2026-08-18 | D1 远端回环端口转发格式修复（待 Actions） | 严格按 run `32083951616` 的一致 formatter 输出机械合并活动计数器调用和无效目标端口错误，未改行为。本机 `git diff --check`、严格 JSON、79-command handler/capability/Android 分离、fixture `bash -n` 与本机 `sshd -T` 配置解析通过；无 Cargo/rustfmt/node_modules/target，根分区 47%。等待修复提交后的四平台 locked fmt/check/test、Linux 真实双跳远端转发 fixture 与 Android 矩阵完整验证。 |
| 2026-08-18 | D1 Linux Actions 依赖安装有界恢复（待 Actions） | 格式修复提交 `32c4904` 的 PR #1 run `32084465003` 中 frontend、Windows/macOS Intel/macOS arm locked fmt/check/test 与 Android aarch64 debug APK/Gradle gate 均成功；Ubuntu runner 自 00:25 UTC 起停在仓库测试之前的系统依赖安装超过 43 分钟，部分日志不可用且仍未进入终态。CI 现对 `apt-get update` 设置 20 秒 HTTP/HTTPS 获取超时、内部与外层三次有界重试，对非交互安装设置 60 秒 dpkg 锁等待和 8 分钟硬超时，整个步骤最多 15 分钟，避免 runner 无限占用并保留明确失败。产品代码未改；本机 `git diff --check`、严格 JSON、安装与双 sshd fixture 两段 `bash -n` 通过，无 Cargo/rustfmt/node_modules/target，根分区 46%。等待新提交的完整矩阵验证。 |

## 下一个动作

等待 D1 Linux Actions 有界依赖安装修复提交的新 PR #1 Actions 全部进入 `COMPLETED/SUCCESS`；失败则继续定位并修复。全绿后实现动态 SOCKS 转发。C4 真机泄漏矩阵保持外部验收；Android Sync capability 继续 disabled。
